...
let read_value_description env parent vd =
  ...
  let locs = read_locations id (Some vd.val_val.val_uid) vd.val_loc in
  let container =
...
let read_type_declaration env parent decl =
  ...
  let locs = read_locations id (Some decl.typ_type.type_uid) decl.typ_loc in
...
let read_extension_constructor env parent ext =
  ...
  let locs = read_locations id (Some ext.ext_type.ext_uid) ext.ext_loc in
...
let read_class_type_declaration env parent cltd =
  ...
  let locs = read_locations id (Some cltd.ci_type_decl.clty_uid) cltd.ci_loc in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
let read_class_description env parent cld =
  ...
  let locs = read_locations id (Some cld.ci_decl.cty_uid) cld.ci_loc in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
