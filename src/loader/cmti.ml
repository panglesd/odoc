...
let read_locations id uid loc =
  Cmi.read_locations (id :> Identifier.t) uid loc
...
let read_value_description env parent vd =
  ...
  let uid = Source_info.Uid.of_value_description vd.val_val in
...
let read_type_declaration env parent decl =
  ...
  let uid = Source_info.Uid.of_type_declaration decl.typ_type in
  let locs = read_locations id uid decl.typ_loc in
...
let read_extension_constructor env parent ext =
  ...
  let uid = Source_info.Uid.of_extension_constructor ext.ext_type in
...
let read_class_type_declaration env parent cltd =
  ...
  let uid = Source_info.Uid.of_class_type_declaration cltd.ci_type_decl in
  let locs = read_locations id uid cltd.ci_loc in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
let read_class_description env parent cld =
  ...
  let uid = Source_info.Uid.of_class_declaration cld.ci_decl in
  let locs = read_locations id uid cld.ci_loc in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
and read_module_type_declaration env parent mtd =
  ...
  let locs = Some (read_locations id None mtd.mtd_loc) in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
and read_module_declaration env parent md =
  ...
  let locs = Some (read_locations id None md.md_loc) in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
