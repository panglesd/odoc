...
let read_locations parent impl intf =
  let source_parent =
  ...
  and impl = match impl with
    | None -> None
    | Some impl -> Some (Source_info.Uid.string_of_uid impl)
  in
  { Locations.source_parent; impl; intf }
...
let read_value_description env parent id vd =
  ...
  let uid = Source_info.Uid.of_value_description vd in
  let locs = read_locations id uid vd.val_loc in
  let container =
...
let read_type_declaration env parent id decl =
  ...
  let uid = Source_info.Uid.of_type_declaration decl in
  let locs = read_locations id uid decl.type_loc in
...
let read_extension_constructor env parent id ext =
  ...
  let uid = Source_info.Uid.of_extension_constructor ext in
  let locs = read_locations id uid ext.ext_loc in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
let read_class_type_declaration env parent id cltd =
  ...
  let uid = Source_info.Uid.of_class_type_declaration cltd in
  let locs = read_locations id uid cltd.clty_loc in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
let read_class_declaration env parent id cld =
  ...
  let uid = Source_info.Uid.of_class_declaration cld in
  let locs = read_locations id uid cld.cty_loc in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
and read_module_type_declaration env parent id (mtd : Odoc_model.Compat.modtype_declaration) =
  ...
  let uid = mtd.mtd_uid in
  let locs = Some (read_locations id uid mtd.mtd_loc) in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
and read_module_declaration env parent ident (md : Odoc_model.Compat.module_declaration) =
  ...
  let locs = Some (read_locations id md.md_uid md.md_loc) in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
