...
let read_locations parent impl intf =
  ...
  and intf = Some (Doc_attr.read_location intf)
      and impl = match impl with None -> None |Some impl -> Some (Source_info.Local_jmp.string_of_uid impl) in
...
let read_value_description env parent id vd =
  ...
  let locs = read_locations id (Some vd.val_uid) vd.val_loc in
  let container =
...
let read_type_declaration env parent id decl =
  ...
  let locs = read_locations id (Some decl.type_uid) decl.type_loc in
...
let read_extension_constructor env parent id ext =
  ...
  let locs = read_locations id (Some ext.ext_uid) ext.ext_loc in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
let read_class_type_declaration env parent id cltd =
  ...
  let locs = read_locations id (Some cltd.clty_uid) cltd.clty_loc in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
let read_class_declaration env parent id cld =
  ...
  let locs = read_locations id (Some cld.cty_uid) cld.cty_loc in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
and read_module_type_declaration env parent id (mtd : Odoc_model.Compat.modtype_declaration) =
  ...
  let locs = Some (read_locations id (Some mtd.mtd_uid) mtd.mtd_loc) in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
...
and read_module_declaration env parent ident (md : Odoc_model.Compat.module_declaration) =
  ...
  let locs = Some (read_locations id (Some md.md_uid) md.md_loc) in
  let container = (parent : Identifier.Signature.t :> Identifier.LabelParent.t) in
