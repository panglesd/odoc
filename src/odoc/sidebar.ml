...
let compile ~page_roots ~lib_roots ~output ~name ~warnings_options:_ =
  let resolver =
    Resolver.create ~important_digests:false ~directories:[]
      ~roots:(Some { page_roots; lib_roots; current_root = name })
      ~open_modules:[]
  in
  let pages = Resolver.all_pages resolver in
  let pages =
    List.map
  ...
  let libraries =
    List.map
      ...
      lib_roots
  in
  let content = { (* package = _;  *) name; pages; libraries } in
  let file = output in
