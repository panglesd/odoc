...
module Odoc_html_args = struct
  ...
  let source_root =
    let doc =
      "Source code root for the compilation unit. It must have been compiled with \
      --source-parent passed."
    in
    Arg.(
      value
  ...
  let extra_args =
    let config semantic_uris closed_details indent theme_uri support_uri flat
        as_json source_file source_root =
      ...
      { Html_page.html_config; source }
    in
    Term.(
      const config $ semantic_uris $ closed_details $ indent $ theme_uri
      $ support_uri $ flat $ as_json $ source_file $ source_root)
end
