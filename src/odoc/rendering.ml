...
let documents_of_asset ~warnings_options:_ ~asset_path unit =
...
let documents_of_odocl ~warnings_options ~renderer ~extra ~syntax ~asset_path
    input =
  let filename = Fpath.filename input in
...
let generate_odoc ~syntax ~warnings_options ~renderer ~output ~extra_suffix
  ...
  >>= fun sidebar ->
  documents_of_odocl ~warnings_options ~renderer ~extra ~syntax ~asset_path file
  >>= fun docs ->
  List.iter
