...
module Occtbl : sig
  ...
  let add tbl id =
    let rec add ?(kind = `Indirect) id =
      ...
      in
      match id.iv with
      | `InstanceVariable (parent, _) -> do_ parent id
      | `Parameter (parent, _) -> do_ parent id
      | `Module (parent, _) -> do_ parent id
      | `ModuleType (parent, _) -> do_ parent id
      | `Method (parent, _) -> do_ parent id
      | `Field (parent, _) -> do_ parent id
      | `Extension (parent, _) -> do_ parent id
      | `Type (parent, _) -> do_ parent id
      | `Constructor (parent, _) -> do_ parent id
      | `Exception (parent, _) -> do_ parent id
      | `Class (parent, _) -> do_ parent id
      | `Value (parent, _) -> do_ parent id
      | `ClassType (parent, _) -> do_ parent id
      | `Root _ -> incr tbl id
