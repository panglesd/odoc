...
(** Raises warnings and errors. *)
let resolve_and_substitute ~resolver ~make_root ~source ~hidden ~search_assets
  ...
  let unit, cmt_infos =
    ...
    | `Cmti ->
        let unit =
          Odoc_loader.read_cmti ~make_root ~parent ~filename ~search_assets
        ...
        (unit, cmt_infos)
    | `Cmt ->
        Odoc_loader.read_cmt ~make_root ~parent ~filename ~search_assets
        |> Error.raise_errors_and_warnings
    | `Cmi ->
        let unit =
    ...
    let result =
      Error.catch_errors_and_warnings (fun () ->
          resolve_and_substitute ~resolver ~make_root ~hidden ~source
            ~search_assets parent input input_type)
    in
