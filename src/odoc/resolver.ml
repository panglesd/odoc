...
let lookup_unit_by_name ap target_name =
  let first_unit u =
    ...
    | Index_content _ | Page_content _ | Source_tree_content _ -> None
  in
...
let lookup_page ap target_name =
  ...
  let is_page u =
    ...
    | Index_content _ | Unit_content _ | Source_tree_content _ -> None
  in
  let units = load_units_from_name ap target_name in
