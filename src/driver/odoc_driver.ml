...
let run libs verbose packages_dir odoc_dir odocl_dir html_dir stats nb_workers
  ...
  List.iter
    (fun l ->
      if Astring.String.is_infix ~affix:"_odoc/ppxlib/doc/page-index.odoc" l
      then Format.printf "%s\n" l)
    !Cmd_outputs.link_output;
  List.iter
    (fun l ->
      if Astring.String.is_infix ~affix:"_odoc/ppxlib/doc/page-index.odocl" l
      then Format.printf "%s\n" l)
    !Cmd_outputs.generate_output;
  List.iter
    (fun l ->
      if Astring.String.is_infix ~affix:"ppxlib" l then Format.printf "%s\n" l)
    !Cmd_outputs.index_output;
