  ...
  let mk_mlds pkg_name odoc_pages =
    ...
    let mlds, assets =
      Fpath.Set.fold
        ...
        odoc_pages ([], [])
    in
    let generated_index_page =
      if
        List.exists
          (fun { mld_rel_path; _ } ->
            Fpath.equal (Fpath.v "index.mld") mld_rel_path)
          mlds
      then []
      else
        let mld_path =
          (* Bos.OS.File.tmp "index_%s.mld" |> Result.get_ok *)
          Fpath.(v "_odoc" / pkg_name / "index.mld")
        in
        let () =
          let _b =
            Bos.OS.Dir.create Fpath.(v "_odoc" / pkg_name) |> Result.get_ok
          in
          Bos.OS.File.write mld_path
            (Format.sprintf
               {|{0 %s}

This page has been autogenerated, it is temporary and should be replaced by Emile's work.

In the meantime, use the sidebar.|}
               pkg_name)
          |> Result.get_ok
        in
        [ { mld_path; mld_rel_path = Fpath.v "index.mld" } ]
    in
    (generated_index_page @ mlds, assets)
  in
